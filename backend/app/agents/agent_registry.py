...
def _build_router_instruction() -> str:
    ...
    return (
        ...
        "# Rules\n"
        "1. Return ONLY the JSON with `target`, `action_name`, `reasoning`, `user_message`.\n"
        "2. `target` MUST be exactly one of the enum values above.\n"
        ...
        "4. When target is 'generic', set `user_message` to a friendly, helpful "
        "response explaining what the app CAN do.\n"
...
_AGENT_CONFIGS: dict[str, dict[str, Any]] = {
    ...
    "reflection": {
        ...
        "instruction": (
            ...
            "Your job is to:\n"
            "1. Update the user profile (expertise level, preferences, style)\n"
            "2. Update project context (goals, tech stack, constraints)\n"
            "3. Record any new learnings or decisions\n"
            "4. Log a brief event for episodic memory\n"
            "5. Identify new goals, blockers, or open questions\n\n"
            "# Rules\n"
            "- Be concise. Each update field should be short.\n"
            "- Only produce updates that are actually warranted — don't repeat "
            "existing state.\n"
            "- For new_goals: only add if the user's message reveals a new intent.\n"
